...
# fileName = QFileDialog.getSaveFileName(self, '保存图片', '图片', "png文件(*.png)")
        fileName = QFileDialog.getSaveFileName(self, '保存图片', '.', ".png")
        if fileName[0]:
            pix.save(fileName[0] + fileName[1])
        else:
            QMessageBox.information(self, "提示", "没有保存图片,请重新保存")
        self.close()
...
# # 初始化MainWindow_JT窗口
# MainWindow_JT = QMainWindow()
# JT_ui = ScreenShotsWin.MyWin()
# JT_ui.initUi()
# # 打开截图窗口
