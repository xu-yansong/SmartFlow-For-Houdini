...
DEFAULT_MULTICAST_GROUP_ENDPOINT = ('239.0.0.1', 6776)  # The multicast group endpoint tuple that the UDP multicast socket should join (must match the "Multicast Group Endpoint" setting in the Python plugin)
        ...
        while self._running:
            ...
            while True:
                try:
                    data = b''
                    while True:
                        part = self._broadcast_socket.recv(DEFAULT_RECEIVE_BUFFER_SIZE)
                        data += part
...
class _RemoteExecutionCommandConnection(object):
    ...
    def _receive_message(self, expected_type):
        ...
        data = b''
        while True:
            part = self._command_channel_socket.recv(DEFAULT_RECEIVE_BUFFER_SIZE)
            data += part
            if len(part) < DEFAULT_RECEIVE_BUFFER_SIZE:
                break
        if data:
